...
| S2 | **Human interference** | `?action=run` (3 tests) | People edit production sheets. Sorting the data range by a non-id column and inserting a blank row mid-table are both survivable (the adapter resolves rows by scanning the id column, and all-empty rows are filtered out). Inserting a **foreign column** mid-table is not survivable by the positional mapping, so since #179 it is refused: every read and write throws `SchemaMismatchError` (`SCHEMA_MISMATCH`) naming the drifted column, and the sheet is left untouched. The same test pins the `skipHeaderCheck: true` escape hatch, which restores the old, documented-dangerous silent misalignment. |
...
| `foreign column` fails with a *missing* `SchemaMismatchError` | The header-drift guard (#179) stopped firing — a read/write entry point lost its `assertHeaderAligned()` call, and column insertion is silently corrupting data again |
| `foreign column` fails on the `skipHeaderCheck` half | The escape hatch no longer bypasses the guard, or the positional read/write it documents has changed shape |
| Any other test fails with `SchemaMismatchError` | Its sheet's physical header drifted from the declared columns — usually a value-level migration that never rewrote the header (see #180), not a guard bug |
