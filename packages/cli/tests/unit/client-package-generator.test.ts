...
describe('generateClientPackage (all files)', () => {
  ...
  it('never references the unusable @gsquery/client/generated subpath', () => {
    const files = generateClientPackage(simpleSchema)

    for (const content of Object.values(files)) {
      expect(content).not.toContain('@gsquery/client/generated')
    }
  })
})

// =============================================================================
// Import Path Documentation (Issue #133)
// =============================================================================

describe('generated import path', () => {
  it('documents the default project-relative import path', () => {
    const result = generateClientCode(simpleSchema)

    expect(result).toContain(`import { createClient } from '${DEFAULT_CLIENT_IMPORT_PATH}'`)
    expect(DEFAULT_CLIENT_IMPORT_PATH).toBe('./generated/client')
  })

  it('documents a custom import path when provided', () => {
    const result = generateClientCode(simpleSchema, { importPath: './src/db' })

    expect(result).toContain("import { createClient } from './src/db'")
    expect(result).not.toContain(`from '${DEFAULT_CLIENT_IMPORT_PATH}'`)
  })

  it('forwards the import path through generateClientPackage', () => {
    const files = generateClientPackage(simpleSchema, { importPath: './src/db' })

    expect(files['client.ts']).toContain("import { createClient } from './src/db'")
  })

  it('escapes quotes in the import path', () => {
    const result = generateClientCode(simpleSchema, { importPath: "./we'ird" })

    expect(result).not.toContain("from './we'ird'")
    expect(result).toContain("./we\\'ird")
  })
})
